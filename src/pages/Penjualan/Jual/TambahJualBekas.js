...
const TambahJualBekas = () => {
  ...
  const [inputTglJual, setInputTglJual] = useState(
    new Date(user.periode.periodeAwal)
  );
  const [jenisJual, setJenisJual] = useState("");
  const [inputTglAng, setInputTglAng] = useState(
    new Date(user.periode.periodeAwal)
  );
  ...
  const countDateDuration = (e) => {
    var tempDate = new Date(inputTglAng);
