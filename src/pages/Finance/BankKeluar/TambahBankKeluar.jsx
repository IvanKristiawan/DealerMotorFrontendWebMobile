...
const TambahBankKeluar = () => {
  ...
  const findDefaultDate = async () => {
    ...
    if (isDateBetween) {
      ...
      setInputTglBankKeluar(new Date());
    }
  };
  ...
  const saveBankKeluar = async (e) => {
    ...
    let tglBankKeluar =
      inputTglBankKeluar?.getFullYear() +
      "-" +
      (inputTglBankKeluar?.getMonth() + 1).toLocaleString("en-US", {
        minimumIntegerDigits: 2,
        useGrouping: false
      }) +
      "-" +
      inputTglBankKeluar?.getDate().toLocaleString("en-US", {
        minimumIntegerDigits: 2,
        useGrouping: false
      });

    let isFailedValidation =
      inputTglBankKeluar === null || kodeCOA.length === 0;
        ...
        <Box sx={textFieldContainer}>
          <Box sx={textFieldWrapper}>
            ...
            <DatePicker
              ...
              selected={inputTglBankKeluar}
              onChange={(e) => setInputTglBankKeluar(e)}
              customInput={
                <TextField
                  error={error && inputTglBankKeluar === null && true}
                  helperText={
                    error &&
                    inputTglBankKeluar === null &&
                    "Tanggal harus diisi!"
                  }
                  sx={{ width: "100%" }}
                  size="small"
