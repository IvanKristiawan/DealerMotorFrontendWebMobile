...
const UbahBankMasukChild = () => {
  ...
  const getBankMasukChildById = async () => {
    setLoading(true);
    const pickedBankMasukChild = await axios.post(
    ...
    setNoBukti(pickedBankMasukChild.data.noBukti);
    setTglBankMasuk(pickedBankMasukChild.data.tglBankMasuk);
    setKodeCOA(pickedBankMasukChild.data.COA.kodeCOA);
    setKeterangan(pickedBankMasukChild.data.keterangan);
    setJumlah(pickedBankMasukChild.data.jumlah);
    setJumlahBaru(pickedBankMasukChild.data.jumlah);
    setLoading(false);
  };
  ...
  const updateBankMasukChild = async (e) => {
    e.preventDefault();
    var date = new Date();
    var current_date =
    ...
    if (isFailedValidation) {
      ...
      try {
        setLoading(true);
        const pickedBankMasuk = await axios.post(
          `${tempUrl}/bankMasuks/${id}`,
          {
            id: user._id,
            token: user.token,
            kodeCabang: user.cabang._id
        ...
        let tempJumlahBankMasuk =
          parseInt(pickedBankMasuk.data.jumlah) -
          parseInt(jumlah) +
          parseInt(jumlahBaru);
        ...
        await axios.post(
          `${tempUrl}/updateBankMasukChild/${idBankMasukChild}`,
          {
            tglBankMasuk,
      ...
      <Paper sx={contentContainer} elevation={12}>
        ...
        <Box sx={textFieldStyle}>
          <Button
            ...
            color="secondary"
            onClick={() =>
              navigate(`/daftarBankMasuk/bankMasuk/${id}/${idBankMasukChild}`)
            }
            sx={{ marginRight: 2 }}
          >
            {"< Kembali"}
...
export default UbahBankMasukChild;
