...
const TambahKasMasuk = () => {
  ...
  const getNextKodeKasMasuk = async () => {
    setLoading(true);
    const nextKodeKasMasuk = await axios.post(`${tempUrl}/kasMasuksNextKode`, {
      id: user._id,
      token: user.token,
  ...
  const saveKasMasuk = async (e) => {
    ...
    let tglKasMasuk =
      inputTglKasMasuk?.getFullYear() +
      "-" +
      (inputTglKasMasuk?.getMonth() + 1).toLocaleString("en-US", {
        minimumIntegerDigits: 2,
        useGrouping: false
      }) +
      "-" +
      inputTglKasMasuk?.getDate().toLocaleString("en-US", {
        minimumIntegerDigits: 2,
        useGrouping: false
      });

    let isFailedValidation = inputTglKasMasuk === null || kodeCOA.length === 0;
    if (isFailedValidation) {
      ...
      try {
        ...
        await axios.post(`${tempUrl}/saveKasMasuk`, {
          tglKasMasuk,
        ...
        <Box sx={textFieldContainer}>
          <Box sx={textFieldWrapper}>
            ...
            <DatePicker
              ...
              onChange={(e) => setInputTglKasMasuk(e)}
              customInput={
                <TextField
                  error={error && inputTglKasMasuk === null && true}
                  helperText={
                    error && inputTglKasMasuk === null && "Tanggal harus diisi!"
                  }
                  sx={{ width: "100%" }}
                  size="small"
                />
