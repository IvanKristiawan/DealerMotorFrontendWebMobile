...
const CariLapPenjualanPerSurveyor = () => {
  ...
  const downloadPdfRekap = async () => {
    ...
    tempHeight += 4;
    doc.line(15, tempHeight, 195, tempHeight);
    doc.save(`laporanPenjualanPerSurveyorRekap.pdf`);
  };
  ...
  const downloadPdfRinci = async () => {
    ...
    doc.text(`${setting.lokasiPerusahaan}`, 15, 15);
    doc.setFontSize(16);
    doc.text(`Laporan Penjualan Per Surveyor Rinci`, 170, 30);
    doc.setFontSize(10);
    doc.text(`Dari Tanggal : ${dariTgl} s/d ${sampaiTgl}`, 15, 40);
    doc.line(15, y, 380, y);
    y += 5;
    ...
    doc.line(380, 55, 380, y);
    doc.line(15, y, 380, y);
    doc.save(`laporanPenjualanPerSurveyorRinci.pdf`);
  };
